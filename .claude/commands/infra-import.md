...
allowed-tools: Read, Write, Edit, Grep, Glob, Bash(python scripts/infra.py:*)
---
# /infra-import — 导入知识
...
2. `python scripts/infra.py new <kind> <中文文件名> --domain <域键> --title "<标题>" --tags "<标签>"` 生成骨架
3. 原样填入材料中的步骤/参数；**缺的信息写 TODO，禁止编造**；playbook 标 symptoms；资源对象同步补 inventory.yaml
4. `python scripts/infra.py lint` 错误清零 → `index` 刷新
